...
def load_production_pipeline():
    try:
        ...
        st.error(
            f"❌ Core Pickled Artifact Missing from `saved_models/`! Run your notebook export block cells first. Details: {str(e)}")
...
sms_input = st.text_area("SMS Body Message Content:",
                         placeholder="Type or paste the text message here...", height=110)
sender_input = st.text_input("Sender String Identification:",
                             placeholder="e.g., +639171234567, 8080, GCash, promo")
date_input = st.text_input("Simulation Timestamp:", value=str(
    pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')))

if st.button("🛡️ Launch Multi-Layer Threat Scan"):
    if not sms_input.strip() or not sender_input.strip():
        st.warning(
            "⚠️ Both the SMS text payload and the Sender ID parameters are required to run an evaluation.")
    else:
        with st.spinner("Processing structural features and conducting multi-model passes..."):
            ...
            new_df["sender_category"] = new_df["sender"].apply(
            ...
            urgency_words = ["voucher", "congratulations", "locked",
                             "winner", "urgent", "account", "verify", "nanalo"]
            new_df["urgency_score"] = new_df["text"].apply(lambda x: sum(
                1 for word in urgency_words if word in str(x).lower()))

            new_df["date-received"] = pd.to_datetime(
                new_df["date-received"], errors="coerce")
            new_df["hour_received"] = new_df["date-received"].dt.hour.fillna(
                0).astype(int)

            # One-Hot Encoding Alignment Mapping
            new_df = pd.get_dummies(
                new_df, columns=["sender_category"], prefix="sc")
            ...
            if expected_meta_features == 5:
                new_meta_X = np.array(
                    [[probs["SVM"], probs["LR"], probs["NB"], probs["RF"], probs["XGB_Base"]]])
            else:
                # Dynamic fallback strategy to your default text-heavy top combination configurations
                new_meta_X = np.array(
                    [[probs["SVM"], probs["LR"], probs["NB"]]])
                if new_meta_X.shape[1] != expected_meta_features:
                    full_stack_list = [probs["SVM"], probs["LR"],
                                       probs["NB"], probs["RF"], probs["XGB_Base"]]
                    new_meta_X = np.array(
                        [full_stack_list[:expected_meta_features]])
            ...
            if prediction == 1:
                ...
                st.metric(label="Meta-Ensemble Threat Confidence Score",
                          value=f"{probability * 100:.2f}%")
            else:
                st.success(f"✅ **SECURE PROFILE: LEGITIMATE MESSAGING (HAM)**")
                st.metric(label="Meta-Ensemble Safety Confidence Score",
            ...
            with st.expander("📊 View Multi-Layer Stacking Diagnostic Metrics"):
                ...
                with col1:
                    ...
                    st.write(
                        f"• Payload Length: {int(new_df['text_len'].iloc[0])} characters")
                    st.write(
                        f"• Malicious URL Detected: {'Yes' if new_df['has_url'].iloc[0] == 1 else 'No'}")
                    st.write(
                        f"• Urgency Keyword Match Hits: {int(new_df['urgency_score'].iloc[0])}")
                    st.write(
                        f"• Sender Node: {new_df.filter(like='sc_').idxmax(axis=1).iloc[0].replace('sc_', '').replace('_', ' ')}")
                with col2:
                    ...
                    st.write(
                        f"• Random Forest (Meta Engine): {probs['RF']:.4f}")
                    st.write(
                        f"• XGBoost Base (Meta Engine): {probs['XGB_Base']:.4f}")
